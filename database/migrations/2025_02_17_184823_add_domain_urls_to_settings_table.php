...
return new class extends Migration
{
    ...
    {
        if(!Schema::connection(config('languages.db_connection'))->hasColumns(config('languages.table_settings'), ['domains'])) {
    ...
    {
        if(Schema::connection(config('languages.db_connection'))->hasColumns(config('languages.table_settings'), ['domains', 'import_only_from_root_language'])) {
            Schema::connection(config('languages.db_connection'))->table(config('languages.table_settings'), function (Blueprint $table) {

...
@section('content')
    ...
    <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg">
        ...
        <div class="flex flex-col md:flex-row items-center justify-end space-y-3 md:space-y-0 md:space-x-4 p-4">
            @if($isAdministrator)
                    @if($githubPrEnabled)
                        <label class="inline-flex items-center cursor-pointer" title="{{ __('languages::translations.create_pr_tooltip') }}">
                            <input type="checkbox" wire:model="createPrOnExport" class="sr-only peer">
                            <div class="relative w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600"></div>
                            <span class="ms-3 text-sm font-medium text-gray-900 dark:text-gray-300">{{ __('languages::translations.create_pr_label') }}</span>
                        </label>
                    @endif
                ...
                <div class="w-full flex flex-wrap gap-4 text-md" wire:loading.remove>
                    ...
                    <div class="w-full mb-4 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-700 dark:border-gray-600">
                        <div class="px-4 py-2 bg-white rounded-t-lg dark:bg-gray-800">
                            @if($translation)
                                <textarea rows="4" wire:model="translatedValue" wire:loading.remove wire:target="openAITranslate"
